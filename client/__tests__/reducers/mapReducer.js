...
describe('reducers - mapReducer', () => {
  ...
  it('should set loading to true', () => {
    expect(mapReducer(state, { type: map.fetchUpdate.pending })).toEqual({
      ...state,
      loading: true
    });

    expect(mapReducer(state, { type: map.fetch.pending })).toEqual({
      ...state,
      loading: true
    });
  });
  ...
  it('should set error', () => {
    ...
    expect(
      mapReducer(state, { type: map.fetchUpdate.rejected, payload: error })
    ).toEqual({ ...state, error });

    expect(
      mapReducer(state, { type: map.fetch.rejected, payload: error })
    ).toEqual({ ...state, error });
  });
});
